...
file_paths=[#'/home/stage/M2-Stage-Liphy/test_colonne_mai_remplissage_8_co2.csv',     ## SANS MOUSSE
            ...
            #'/home/stage/M2-Stage-Liphy/Tests_colonne_mousse_3D/test_colonne_mousse_3D_coloree_040324_remplissage_8_co2.csv',    ### 5 g/L SDS
            ...
            '/home/stage/M2-Stage-Liphy/remplissage_aspiro_300ml_3point5_8_co2.csv',   ### ASPIRO
...
for file_path in file_paths:
    ...
    if 'plus' in file_path:
        if 'vidange' in file_path:
            label='Vidange 1.7 g/L (70% CMC) 600 ml'
            y=(db['Media value concentration']/100)/(db['Media value concentration'].max()/100)
            plt.scatter(x[::300], y[::300], label=f'{label} {percentage*10} ml/min CO2', color='m', marker='*', s=10)
        else:
            label='Remplissage 1.7 g/L (70% CMC) 600 ml'
            y=(db['Media value concentration']/100)/(db['Media value concentration'].max()/100)
            plt.scatter(x[::300], y[::300], label=f'{label} {percentage*10} ml/min CO2', color='m', marker='o', s=10)
        
    if 'coloree' in file_path:
        if 'vidange' in file_path:
            label='Vidange 5 g/L (colorant) SDS'
            plt.scatter(x[::300], y[::300], label=f'{label} {percentage*10} ml/min CO2', color='m', marker='*', s=10)
        else:
            label='Remplissage 5 g/L (colorant) SDS'
            plt.scatter(x[::300], y[::300], label=f'{label} {percentage*10} ml/min CO2', color='m', marker='o', s=10)
    if '5gL' in file_path:
        if 'vidange' in file_path:
            label='Vidange 5 g/L SDS 2eme experience'
            #y=(db['Media value concentration']/100)/(db['Media value concentration'].max()/100)
            plt.scatter(x, y, label=f'{label} {percentage*10} ml/min CO2', color='r', marker='x', s=1)
        else:
            label='Remplissage 5 g/L 2eme experience'
            #y=(db['Media value concentration']/100)/(db['Media value concentration'].max()/100)
            plt.scatter(x, y, label=f'{label} {percentage*10} ml/min CO2', color='r', marker='v', s=1)  
    
    if 'cinqgramesparlittre' in file_path:
        if 'vidange' in file_path:
            label='Vidange 5 g/L 600 ml SDS'
            plt.scatter(x[::300], y[::300], label=f'{label} {percentage*10} ml/min CO2', color='m', marker='*', s=10)
        else:
            label='Remplissage 5 g/L 600 ml SDS'
            plt.scatter(x[::300], y[::300], label=f'{label} {percentage*10} ml/min CO2', color='m', marker='o', s=10)
    if 'aspiro' in file_path:
        if 'vidange' in file_path:
            label='Vidange aspiro 0.35 g/L (70% CMC) 300ml'
            plt.scatter(x, y, label=f'{label} {percentage*10} ml/min CO2', color='g', marker='x', s=1)
        else:
            label='Remplissage aspiro 0.35 g/L (70% CMC) 300 ml'
            plt.scatter(x, y, label=f'{label} {percentage*10} ml/min CO2', color='g', marker='v', s=1) 
